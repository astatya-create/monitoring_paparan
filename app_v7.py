...
def tambah_bahan_dialog() -> None:
    ...
    with st.form("form_tambah_bahan", clear_on_submit=True):
        ...
        with col1:
            ...
            pic_options = [p for p in PIC_BY_KANTOR.get(kantor, []) if p in daftar_pic]
            if not pic_options:
                pic_options = PIC_BY_KANTOR.get(kantor, [])
            pic1 = st.selectbox("PIC 1", pic_options)
        with col2:
            deadline = st.date_input("Deadline")
            jenis = st.selectbox("Jenis Bahan", JENIS_OPTIONS)
            pic2_candidates = [p for p in pic_options if p != pic1]
            if not pic2_candidates:
                pic2_candidates = pic_options
            pic2 = st.selectbox("PIC 2", pic2_candidates)
...
def render_charts(df: pd.DataFrame, tahun_pilih: int) -> None:
    ...
    with c4:
        ...
        if df_trend["tgl_disposisi"].notna().any():
            ...
            fig = px.bar(bulanan_view, x="Bulan", y="Jumlah", text="Jumlah")
            fig.update_traces(textposition="outside")
            fig.update_layout(showlegend=False, yaxis=dict(dtick=1), margin=dict(l=10, r=10, t=10, b=10), height=360)
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    for no, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        with c1:
            ...
            if deadline_days is not None and deadline_days <= 3:
                label = "Terlambat" if deadline_days < 0 else f"H-{deadline_days}"
