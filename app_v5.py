...
def inject_dashboard_css() -> None:
    st.markdown(
        ...
        [data-testid="stSidebar"] .stButton > button[kind="primary"] {
            ...
            color: white !important;
            border: 1px solid #17355c !important;
        }
...
def sort_bahan_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sorted_df["is_proses"] = sorted_df["status_normalized"].isin(["on progress", "proses", "process"])

    df_proses = sorted_df[sorted_df["is_proses"]].copy()
    df_lain = sorted_df[~sorted_df["is_proses"]].copy()

    if not df_proses.empty:
        df_proses = df_proses.sort_values(
            by=["deadline", "tgl_disposisi", "id"],
    ...
    final_df = pd.concat([df_proses, df_lain], ignore_index=True)
    return final_df.drop(columns=["status_normalized", "is_proses"], errors="ignore")
