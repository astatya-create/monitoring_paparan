...
def inject_global_css() -> None:
    st.markdown(
        ...
        .header-wrap {
            ...
            background: white; padding: 16px 2rem; border-bottom: 1px solid #e5e7eb;
        }
        .header-title { font-size: 30px; font-weight: 800; color: #0f172a; margin: 0; }
        .header-sub { font-size: 18px; font-weight: 700; color: #64748b; margin: 0; }
        .kpi-card {
            background: white; padding: 18px; border-radius: 18px;
            box-shadow: 0 4px 16px rgba(15,23,42,0.06); margin-bottom: 10px;
        }
        ...
        .section-title { color:#0c4a6e; font-size:20px; font-weight:800; margin:20px 0 10px; }
        .table-card {
            background:white; border-radius:18px; padding:18px;
            box-shadow:0 4px 16px rgba(15,23,42,0.06); margin-top: 18px;
        }
        .badge {
...
def render_tambah_bahan() -> None:
    ...
    st.sidebar.markdown("---")
    st.sidebar.subheader("Tambah Bahan")
    ...
    with st.sidebar.form("form_tambah_bahan", clear_on_submit=True):
        ...
        if daftar_pic:
            ...
            instruksi = st.text_area("Keywords / Instruksi")
    ...
    if not existing.empty:
        st.sidebar.error("Agenda dengan nama bahan dan deadline tersebut sudah ada.")
        return
...
def render_header() -> None:
    logo_html = (
        f'<img src="data:image/png;base64,{file_to_base64(LOGO_PATH)}" width="88">'
        if LOGO_PATH.exists()
        else ""
    )
    st.markdown(
        ...
        <div class="header-wrap">
            {logo_html}
            <div>
                <p class="header-title">Dashboard Monitoring Bahan Paparan Pimpinan</p>
                <p class="header-sub">Bagian PDSIA</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_table(df: pd.DataFrame) -> None:
    ...
    header_cols = st.columns([0.5, 3, 1.3, 1.5, 1.2, 1.1, 1.2, 1.1])
    ...
    for no, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        c0, c1, c2, c3, c4, c5, c6, c7 = st.columns([0.5, 3, 1.3, 1.5, 1.2, 1.1, 1.2, 1.1])
        c0.write(no)
        with c1:
        ...
        with c2:
            ...
            if surat and surat.exists():
                url = f"?preview_id={int(row['id'])}&kind=surat"
                st.markdown(f"[📩 Disposisi]({quote(url, safe='?=&')})")
        with c3:
        ...
        with c6:
            ...
            if paparan and paparan.exists():
                st.markdown(f"[👁️ Paparan](?preview_id={int(row['id'])}&kind=paparan)")
            else:
                st.caption("Paparan: -")
            if narasi and narasi.exists():
                st.markdown(f"[📝 Narasi](?preview_id={int(row['id'])}&kind=narasi)")
            else:
                st.caption("Narasi: -")
        with c7:
