...
def upsert_user(username: str, password: str, role: str, display_name: str, active: str = "1") -> None:
    df = load_users()
    username = username.strip()
    if not username:
        return
    idx = df.index[df["username"].astype(str).str.strip() == username]
    if len(idx) == 0:
        new_row = pd.DataFrame([{
            "username": username,
            "password": password,
        ...
        df.at[i, "role"] = role
        df.at[i, "display_name"] = display_name or username.title()
        df.at[i, "active"] = active
    save_users(df)

def change_my_password(username: str, current_password: str, new_password: str) -> tuple[bool, str]:
    df = load_users()
    idx = df.index[df["username"].astype(str).str.strip() == username.strip()]
    if len(idx) == 0:
        return False, "User tidak ditemukan."
    i = idx[0]
    if str(df.at[i, "password"]) != current_password:
        return False, "Password saat ini salah."
    if len(new_password) < 6:
        return False, "Password baru minimal 6 karakter."
    df.at[i, "password"] = new_password
    save_users(df)
...
with st.sidebar:
    ...
    if st.session_state.get("show_change_password"):
        with st.expander("Form Ganti Password", expanded=True):
            current_pw = st.text_input("Password saat ini", type="password", key="cp_current")
            new_pw = st.text_input("Password baru", type="password", key="cp_new")
            confirm_pw = st.text_input("Konfirmasi password baru", type="password", key="cp_confirm")
            c1, c2 = st.columns(2)
            if c1.button("Simpan Password", use_container_width=True):
                if new_pw != confirm_pw:
                    st.error("Konfirmasi password tidak sama.")
                else:
                    ok, msg = change_my_password(username, current_pw, new_pw)
                    if ok:
                        st.success(msg)
                        ...
                        st.error(msg)
            if c2.button("Tutup Form", use_container_width=True):
                st.session_state["show_change_password"] = False
                st.rerun()

    if role == "admin":
        with st.expander("Kelola User", expanded=False):
            mode = st.radio("Mode", ["Tambah User", "Edit User"], horizontal=True)
            if mode == "Tambah User":
                new_username = st.text_input("Username baru", key="new_username")
                new_password = st.text_input("Password awal", type="password", key="new_password")
                new_role = st.selectbox("Role user", ["pic", "atasan", "admin"], key="new_role")
                new_name = st.text_input("Nama tampil", key="new_name")
                if st.button("Simpan User", use_container_width=True):
                    if not new_username.strip() or not new_password:
                        st.error("Username dan password wajib diisi.")
                    elif new_username.strip() in users_df["username"].astype(str).str.strip().tolist():
                        st.error("Username sudah ada.")
                    else:
                        upsert_user(new_username.strip(), new_password, new_role, new_name, "1")
                        st.success("User berhasil ditambahkan.")
                        st.rerun()
            else:
                user_options = users_df["username"].astype(str).tolist()
                selected_user = st.selectbox("Pilih user", user_options, key="edit_user_pick")
                picked_df = users_df[users_df["username"].astype(str) == selected_user]
                if not picked_df.empty:
                    row_u = picked_df.iloc[0]
                    edit_name = st.text_input("Nama tampil", value=str(row_u["display_name"]), key="edit_name")
                    edit_role = st.selectbox(
                        "Role",
                        ["pic", "atasan", "admin"],
                        index=["pic", "atasan", "admin"].index(str(row_u["role"])) if str(row_u["role"]) in ["pic", "atasan", "admin"] else 0,
                        key="edit_role",
                    )
                    edit_password = st.text_input("Password baru (opsional)", type="password", key="edit_password")
                    edit_active = st.selectbox("Status user", [("1", "Aktif"), ("0", "Nonaktif")], format_func=lambda x: x[1], key="edit_active")
                    if st.button("Update User", use_container_width=True):
                        upsert_user(selected_user, edit_password, edit_role, edit_name, edit_active[0])
